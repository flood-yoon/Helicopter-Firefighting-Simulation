...
public class GameManager : MonoBehaviour
{
    ...
    void StartRound()
    {
        ...
        int fireCount = 2 * (int)Mathf.Pow(2, round - 1);
        ...
        // 파티클 각도/양 라운드마다 증가 (각도 최대 120도, 양 1.5배)
        if (waterSpray != null)
        {
            float angle = baseAngle * Mathf.Pow(1.5f, round - 1);
            float emissionRate = baseEmissionRate * Mathf.Pow(1.5f, round - 1);
            waterSpray.ApplyParticleSettings(angle, emissionRate);
        }
    ...
    void SpawnFire(float health)
    {
        ...
        {
            GameObject fire = Instantiate(firePrefab, hit.point, Quaternion.identity);
